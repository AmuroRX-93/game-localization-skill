...
| Kimi Code | `~/.kimi-code/skills/game-localization/`；设置 `KIMI_CODE_HOME` 时使用其下的 `skills/` | 有专用目录包，客户端内未实测 |
| Qwen Code（通义） | `.qwen/skills/game-localization/` 或 `~/.qwen/skills/game-localization/` | 有专用目录包，客户端内未实测 |
| CodeBuddy（腾讯） | `.codebuddy/skills/game-localization/` 或 `~/.codebuddy/skills/game-localization/` | 有专用目录包，客户端内未实测 |

推荐下载总包，按工具选择文件夹；也提供各工具单独安装包。安装说明：[Kimi Code](KIMI.md)、[Qwen Code](QWEN.md)、[CodeBuddy](CODEBUDDY.md)。保留完整 `game-localization` 文件夹，按说明选择安装目录。`agents/openai.yaml` 是 Codex 界面元数据，核心说明、参考资料与 Python 脚本不依赖它。工具无法加载技能时，可以显式要求它读取 `SKILL.md` 及相关参考，但这不等于自动发现已成功。
