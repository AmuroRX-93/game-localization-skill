...
支持日文→中文、英文→中文。一个压缩包包含五种工具的完整技能、参考资料、检查脚本和各自安装说明。**按你使用的工具选一个文件夹，不要把总包整个放进技能扫描目录。**
...
这是可复用的汉化工程方法，不是所有游戏通用的解包器。Python 检查器验证清单内部矛盾，不能证明全游戏无遗漏或翻译质量。脚本需要 Python 3.9+。
