...
这是可复用的汉化工程方法，不是所有游戏通用的解包器。Python 检查器验证清单内部矛盾，不能证明全游戏无遗漏或翻译质量。脚本需要 Python 3.9+。v0.6.0 新增的可执行工具、依赖诊断和小样见技能内 `references/toolkit.md`；先运行 `scripts/localization_toolkit.py doctor` 和 `scripts/toolkit_selftest.py`。核心只依赖标准库，图像/字体功能按需安装 `scripts/requirements-ui.txt`。
